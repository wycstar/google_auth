...
static void displayEnrollInfo(const char *secret, const char *label,
    ...
    if (isatty(STDOUT_FILENO)) {
        if (!displayQRCode(url)) {
            printf("Failed to use libqrencode to show QR code visually for "
                   "scanning.\n"
...
static void print_version() { puts("google-authenticator " VERSION); }
...
int main(int argc, char *argv[]) {
    ...
    strcpy((char *)buf, "infervision-hospital-reader-wyc");
