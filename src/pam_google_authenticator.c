...
static const char *get_user_name(pam_handle_t *pamh, const Params *params) {
    ...
    if (params->debug) {
        log_message(LOG_INFO, pamh,
                    "debug: start of google_authenticator for \"%s\"",
                    username);
    }
    return username;
}
...
static const char *get_rhost(pam_handle_t *pamh, const Params *params) {
    ...
    if (pam_get_item(pamh, PAM_RHOST, &rhost) != PAM_SUCCESS) {
        log_message(LOG_ERR, pamh,
                    "pam_get_rhost() failed to get the remote host");
        return NULL;
    }
    if (params->debug) {
        log_message(LOG_INFO, pamh,
                    "debug: google_authenticator for host \"%s\"", rhost);
    }
...
static char *get_secret_filename(pam_handle_t *pamh, const Params *params,
    ...
    if (!params->fixed_uid) {
        ...
        *uid = -1;
        ...
        if (rc) {
            log_message(LOG_ERR, pamh, "getpwnam_r(\"%s\")!=0: %d", username,
                        rc);
            goto errout;
        }
        ...
        if (!pw->pw_dir) {
            log_message(LOG_ERR, pamh, "user(\"%s\") has no home dir",
                        username);
            goto errout;
        }

        if (*pw->pw_dir != '/') {
            log_message(LOG_ERR, pamh, "User \"%s\" home dir not absolute",
                        username);
            goto errout;
        }
    }
    ...
    if ((secret_filename = strdup(spec)) == NULL) {
        log_message(LOG_ERR, pamh, "Short (%d) mem allocation failed",
                    strlen(spec));
        goto errout;
    }
    int allow_tilde = 1;
    for (int offset = 0; secret_filename[offset];) {
        ...
        if (var) {
            ...
            if (subst_len > 1000000) {
                log_message(LOG_ERR, pamh, "Unexpectedly large path name: %d",
                            subst_len);
                goto errout;
            }
...
static int open_secret_file(pam_handle_t *pamh, const char *secret_filename,
    ...
    if (fd < 0 || fstat(fd, orig_stat) < 0) {
        if (params->nullok != NULLERR && errno == ENOENT) {
            ...
            // that this is OK. We still return an error from
            // open_secret_file(), but we remember that this was the result of a
            // missing state file.
    ...
    if (orig_stat->st_mode & 03777 & ~params->allowed_perm) {
        log_message(LOG_ERR, pamh,
                    ...
                    secret_filename, orig_stat->st_mode & 03777,
                    params->allowed_perm);
        goto error;
    }
    ...
    if (orig_stat->st_size < 1 || orig_stat->st_size > 64 * 1024) {
        log_message(LOG_ERR, pamh, "Invalid file size for \"%s\"",
                    secret_filename);
        goto error;
    }
...
static int write_file_contents(pam_handle_t *pamh, const Params *params,
                               ...
                               struct stat *orig_stat, const char *buf) {
    int err = 0;
    int fd = -1;
    ...
    if (fnlength - 1 !=
        snprintf(tmp_filename, fnlength, "%s~XXXXXX", secret_filename)) {
        err = ERANGE;
        goto cleanup;
    }
...
static int set_cfg_value(pam_handle_t *pamh, const char *key, const char *val,
    ...
    if (total_len <= stop - start) {
        // We are decreasing out space requirements. Shrink the buffer and pad
        // with NUL characters.
...
static int rate_limit(pam_handle_t *pamh, const char *secret_filename,
    ...
    char *list;
    {
        ...
        for (int i = start; i <= stop; ++i) {
            prnt +=
                snprintf(prnt, list_size - (prnt - list), " %u", timestamps[i]);
        }
        free(timestamps);
    }
...
static int check_scratch_codes(pam_handle_t *pamh, const Params *params,
    ...
    for (;;) {
        ...
        // Skip any lines starting with double-quotes. They contain option
        // fields
        if (*ptr == '"') {
            ptr += strcspn(ptr, "\n");
            continue;
 ...
 */
static int check_time_skew(pam_handle_t *pamh, int *updated, char **buf,
                           int skew, int tm) {
    int rc = -1;
    ...
    if (num_entries == sizeof(tms) / sizeof(int)) {
        ...
        for (int i = 1; i < sizeof(tms) / sizeof(int); ++i) {
            // Check that we have a consecutive sequence of timestamps with no
            // big gaps in between. Also check that the time skew stays
            // constant. Allow a minor amount of fuzziness on all parameters.
        ...
        *reset = '\000';
        if (rc) {
            for (int i = 0; i < num_entries; ++i) {
                ...
                snprintf(pos, reset_size - (pos - reset), " %d%+d" + !*reset,
                         tms[i], skews[i]);
            }
        }
...
static int check_timebased_code(pam_handle_t *pamh, const char *secret_filename,
    ...
    if (!is_totp(*buf)) {
        // The secret file does not actually contain information for a
        // time-based code. Return to caller and see if any other authentication
        // methods apply.
        return 1;
    }
    ...
    for (int i = -((window - 1) / 2); i <= window / 2; ++i) {
        const unsigned int hash =
            compute_code(secret, secretLen, tm + skew + i);
        if (hash == (unsigned int)code) {
            return invalidate_timebased_code(tm + skew + i, pamh,
                                             secret_filename, updated, buf);
        }
    }
    ...
    if (!params->noskewadj) {
        ...
        for (int i = 0; i < 25 * 60; ++i) {
            ...
            if (hash == (unsigned int)code && skew == 1000000) {
                // Don't short-circuit out of the loop as the obvious difference
                // in computation time could be a signal that is valuable to an
                // attacker.
                skew = -i;
            }
...
int update_logindetails(pam_handle_t *pamh, const Params *params, char **buf) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        const int scanf_rc =
            sscanf(line, " %255[0-9a-zA-Z:.-] %lu ", host, &when);
        free(line);
    ...
    if (set_cfg_value(pamh, name, value, buf) < 0) {
        log_message(LOG_WARNING, pamh,
                    "Failed to set cfg value for login host");
    }
...
static int check_counterbased_code(pam_handle_t *pamh,
    ...
    if (hotp_counter < 1) {
        ...
        // counter-based code. Return to caller and see if any other
        // authentication methods apply.
        return 1;
    }
    ...
    // Compute [window_size] verification codes and compare them with user
    // input. Future codes are allowed in case the user computed but did not use
    // a code.
    const int window = window_size(pamh, secret_filename, *buf);
    if (!window) {
        return -1;
    ...
    for (int i = 0; i < window; ++i) {
        const unsigned int hash =
            compute_code(secret, secretLen, hotp_counter + i);
        if (hash == (unsigned int)code) {
            char counter_str[40];
            snprintf(counter_str, sizeof counter_str, "%ld",
                     hotp_counter + i + 1);
...
static int google_authenticator(pam_handle_t *pamh, int argc,
                                const char **argv) {
    int rc = PAM_AUTH_ERR;
    uid_t uid = -1;
    ...
    // Read and process status file, then ask the user for the verification
    // code.
    int early_updated = 0, updated = 0;

    const char *const username = get_user_name(pamh, &params);
    char *const secret_filename =
        get_secret_filename(pamh, &params, username, &uid);
    ...
    // Drop privileges.
    {
        ...
        if (uid == -1) {
            ...
            if (parse_user(pamh, drop_username, &uid)) {
                // If 'nobody' doesn't exist, bail. We need to drop privs to
                // *someone*.
                goto out;
            }
        }
    ...
    if (secret_filename) {
        ...
        if (buf) {
            if (rate_limit(pamh, secret_filename, &early_updated, &buf) >= 0) {
                secret = get_shared_secret(pamh, &params, secret_filename, buf,
                                           &secretLen);
            } else {
    ...
    if (!stopped_by_rate_limit && (secret || params.nullok != SECRETNOTFOUND)) {
        ...
        for (int mode = 0; mode < 4; ++mode) {
            ...
            // get the verification code from the system password or from
            // prompting the user. We need to attempt both. This only works
            // correctly, if all failed attempts leave the global state
            // unchanged.
            if (updated || pw) {
                // Oops. There is something wrong with the internal logic of our
                // code. This error should never trigger. The unittest checks
                // for this.
                if (pw) {
            ...
            if (secret) {
                ...
                switch (check_scratch_codes(pamh, &params, secret_filename,
                                            &updated, buf, code)) {
                case 1:
                    if (hotp_counter > 0) {
                        switch (check_counterbased_code(
                            pamh, secret_filename, &updated, &buf, secret,
                            secretLen, code, hotp_counter,
                        ...
                        switch (check_timebased_code(
                            pamh, secret_filename, &updated, &buf, secret,
                            secretLen, code, &params)) {
                        case 0:
                            rc = PAM_SUCCESS;
                            break;
                        case 1:
        ...
        if (rc == PAM_SUCCESS) {
            log_message(LOG_INFO, pamh, "Accepted google_authenticator for %s",
                        username);
            ...
            log_message(LOG_ERR, pamh, "Invalid verification code for %s",
                        username);
        }
    }
    ...
    if (early_updated || updated) {
        int err;
        if ((err = write_file_contents(pamh, &params, secret_filename,
                                       &orig_stat, buf))) {
            ...
            // If allow_readonly parameter is defined than ignore write errors
